...
export default function JobHistoryDetailPage() {
  ...
  const fetchApplication = async () => {
    try {
      ...
      console.warn("Direct application fetch failed, falling back to history list lookup:", err)
      try {
        // 2. Fallback: Query employment records list and search for matching ID
        const res = await applicationService.getEmploymentRecords({ page: 1, pageSize: 50 })
        ...
        gooeyToast.error("فشل تحميل تفاصيل سجل التوظيف")
      }
    } finally {
      setLoading(false)
    }
  ...
  const handleAction = async (action: 'reject' | 'hire') => {
    ...
    try {
      ...
      // Invalidate query client cache for applications, employment records, and interviews
  ...
  if (!application) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center gap-4 text-center">
        ...
        <h3 className="text-xl font-bold text-foreground">لم يتم العثور على سجل التوظيف</h3>
        <p className="text-muted-foreground max-w-md">
          قد يكون هذا السجل غير موجود أو تم حذفه، أو لا تملك الصلاحية الكافية لعرضه.
        </p>
        <Button onClick={() => router.push('/company/job-history')} variant="default" className="mt-2">
          العودة لسجل التوظيف
        </Button>
      </div>
    )
    ...
    >
      ...
      <div className="border-border/40 from-primary/10 via-background to-background dark:from-primary/5 relative overflow-hidden rounded-2xl border bg-gradient-to-l p-6 shadow-sm">
        <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-foreground text-2xl font-extrabold tracking-tight">تفاصيل سجل التوظيف</h1>
              <p className="text-muted-foreground mt-1 text-sm">عرض معلومات سجل التوظيف وتفاصيل المتقدم التاريخية</p>
            </div>
          </div>
          <Button
            variant="ghost"
            onClick={() => router.push('/company/job-history')}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <Card className="border-border/50 md:col-span-1 shadow-sm h-fit overflow-hidden">
          ...
          <CardContent className="pt-0 pb-6 flex flex-col items-center text-center -mt-10">
            ...
            <div className="w-full space-y-4 text-right">
              ...
              <div>
                <span className="text-muted-foreground text-xs font-semibold block mb-1">حالة التوظيف النهائية</span>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {/* Action Operations card (if applicable in history) */}
